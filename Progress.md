...
- Status: Phase 11/12 COMPLETE (gates 11/12 PASS, full suite 653 passed). UI redesign advancing; Tailwind content-glob root cause fixed. Baseline commit+push `5e90a86` done (2026-09-15); charcoal "Ink" chrome wave IN PROGRESS.
...
## Font system swap — Cairo / Ubuntu / Gruppo / JetBrains Mono (2026-09-15)
- User directive: replace Inter + Space Grotesk. Small text prefer **Cairo** or Ubuntu; medium text orchestrator's choice → **Ubuntu**; very-large text → **Gruppo** (prewire only — no landing/hero page exists yet); mono not in swap set → **JetBrains Mono kept** (code/technical).
- Mapping applied: `--font-sans` → Cairo (body/small/UI/pills/nav); `--font-display` → Ubuntu (page h1 titles, section headings); new `--font-brand` → Gruppo (very-large role; currently used ONLY for the "Smorx" wordmark, weight 400, `auto` tracking 0.12em, uppercase); `--font-mono` → JetBrains Mono (unchanged). Added `.font-brand` utility + Tailwind `brand` family.
- Files: `app/layout.tsx` (next/font imports + html classes), `app/globals.css` (font tokens, `.font-display` 0.01em, `.font-brand`, body `letter-spacing: 0.015em`), `tailwind.config.ts` (fontFamily sans/display/brand/mono), `shell/Navigation.tsx` (Smorx h1 → `font-brand`).
- Slight letter-spacing per user follow-up ("slightly leave space between fonts"): body `0.015em`, `.font-display` `0.01em` (was `-0.01em`).
- Browser verification (DevTools MCP): body computed `Cairo`; wordmark computed `Gruppo` (16.8px, tracking 2.016px); page title computed `Ubuntu` (25.6px); nav labels `letterSpacing 0.24px`, titles `0.256px`; mono numerals still JetBrains Mono + tabular-nums; **console 0 messages**. Typecheck exit 0.
